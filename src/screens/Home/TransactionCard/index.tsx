...
export const TransactionCard: FC<Params> = ({
    transaction
}) => {
    ...
    return (
        ...
        >
            <View className="h-[140] bg-background-tertiary rounded-[6] p-6">
                ...
                <Text
                    className={clsx("text-2xl font-bold mt-2 ", isExpense ? "text-accent-red" : "text-accent-brand-light")}
                >{transaction.type.id === TransactionTypes.EXPENSE && "-"}
                    R$ {transaction.value.toFixed(2).replace(".", ",")}</Text>
                <View className="flex-row w-full justify-between items-center">
                    ...
                    <View className="items-center flex-row mt-3">
                        <MaterialIcons color={colors.gray[700]} size={20} name="calendar-month" />
                        <Text className="text-gray-700 text-base ml-2">{format(transaction.createdAt, "dd/MM/yyyy")}</Text>
                    </View>
                </View>
            </View>
        </Swipeable>
